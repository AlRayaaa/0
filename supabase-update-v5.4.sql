-- الراية للعطور V5.4 — تحديث آمن لقاعدة بيانات موجودة
-- شغّل هذا الملف مرة واحدة في Supabase SQL Editor. لا يحذف المنتجات أو المبيعات أو الحسابات.
...
create or replace function public.record_sale(p_product_id uuid,p_qty numeric,p_price numeric) returns void
...
begin
  ...
  update public.products set qty=p.qty-p_qty,updated_by=auth.uid(),updated_at=now() where id=p_product_id;
  insert into public.sales(product_id,name,qty,total,buy,created_by) values(p.id,p.name,p_qty,p_qty*p_price,p.buy,auth.uid());
end;
$$;
grant execute on function public.record_sale(uuid,numeric,numeric) to authenticated;

create or replace function public.record_purchase(p_product_id uuid,p_qty numeric,p_price numeric,p_dealer text default '',p_invoice text default '',p_note text default '') returns void
language plpgsql security invoker set search_path=public as $$
declare p public.products%rowtype;
begin
  if auth.uid() is null then raise exception 'AUTH_REQUIRED'; end if;
  ...
  select * into p from public.products where id=p_product_id for update;
  if not found then raise exception 'PRODUCT_NOT_FOUND'; end if;
  update public.products set qty=p.qty+p_qty,buy=p_price,updated_by=auth.uid(),updated_at=now() where id=p_product_id;
  insert into public.purchases(product_id,name,qty,price,dealer,invoice,note,created_by) values(p.id,p.name,p_qty,p_price,coalesce(p_dealer,''),coalesce(p_invoice,''),coalesce(p_note,''),auth.uid());
end;
$$;
